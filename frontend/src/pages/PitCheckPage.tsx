...
export default function PitCheckPage() {
  ...
  const [mode, setMode] = useState<Mode>(() =>
    searchParams.get("mode") === "demo" ? "demo" : "select",
  );
  ...
  const handlePasteSubmit = () => {
    ...
    runAnalysis(
      quoteText.trim(),
      Object.values(vehicle).some(Boolean) ? vehicle : undefined,
    );
  };
  ...
  const handleImageSubmit = () => {
    ...
    analyzeImage(
      ...
      .then((result: AnalyzeResponse) =>
        navigate("/briefing", {
          state: { result, quoteText: "[Uploaded Image]", vehicle },
        }),
      )
      .catch((err: unknown) => {
        setErrorMsg(
        ...
        <main className="flex-1 px-6 py-12 max-w-3xl mx-auto w-full">
          <div className="mb-8 fade-in-1">
            ...
            <p className="text-text-secondary text-sm mt-2">
              Choose a realistic shop scenario to see PitWall in action.
            </p>
          </div>
            ...
            {scenarios.map((s, i) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="font-sans font-semibold text-text-primary text-sm mb-1 group-hover:text-accent transition-colors">
                      {s.title}
                    </div>
      ...
      <div className="min-h-screen carbon-bg flex flex-col">
        ...
        <main className="flex-1 px-6 py-12 max-w-2xl mx-auto w-full">
          <div className="mb-8 fade-in-1">
            ...
            <p className="text-text-secondary text-sm mt-2">
              Paste the estimate text, mechanic notes, or anything you were
              told.
            </p>
          </div>
      ...
      <div className="min-h-screen carbon-bg flex flex-col">
        ...
        <main className="flex-1 px-6 py-12 max-w-2xl mx-auto w-full">
          <div className="mb-8 fade-in-1">
            ...
            <h2 className="font-sans font-bold text-2xl text-text-primary">
              Got a photo of the quote?
            </h2>
            <p className="text-text-secondary text-sm mt-2">
              Upload a repair estimate, invoice, or mechanic printout.
            </p>
          </div>
            ...
            >
              ...
              {imageFile ? (
                <div className="text-center px-6">
                  ...
                  <p className="font-sans font-semibold text-text-primary text-sm truncate max-w-xs">
                    {imageFile.name}
                  </p>
                  <p className="font-mono text-xs text-text-secondary mt-1">
                    {formatBytes(imageFile.size)}
                  </p>
                  <p className="font-mono text-xs text-accent/60 mt-2">
                    Click to change
                  </p>
                </div>
              ) : (
                <div className="text-center px-6">
                  ...
                  <p className="font-mono text-xs text-accent/50 mt-3 tracking-wider">
  ...
  return (
    <div className="min-h-screen carbon-bg flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-md border border-[#FF1801]/30 rounded-lg p-6 bg-[#141414] text-center">
        <div className="font-mono text-xs text-[#FF1801] tracking-widest uppercase mb-3">
          Analysis Failed
        </div>
        ...
        <div className="flex gap-3 justify-center">
          <button
            ...
            className="px-5 py-2 border border-[#2a2a2a] text-text-primary font-sans text-sm rounded hover:border-accent transition-all"
          >
            Try Again
          </button>
          <button
            onClick={() => setMode("demo")}
            className="px-5 py-2 bg-accent text-[#0d0d0d] font-sans font-bold text-sm rounded hover:bg-accent/90 transition-all"
          >
            Use Demo
          </button>
        </div>
...
function PageHeader({
  ...
  return (
    <nav className="flex items-center justify-between px-6 py-4 border-b border-[#2a2a2a]">
      ...
      <button
        onClick={onHome}
        className="font-mono text-accent text-sm font-bold tracking-widest hover:opacity-80 transition-opacity"
      >
        PITWALL
      </button>
      <div className="w-16" />
...
function ModeCard({
  ...
  return (
    ...
    >
      ...
      <div className="font-sans font-bold text-text-primary text-lg mb-2 group-hover:text-accent transition-colors">
        {label}
      </div>
      <div className="text-text-secondary text-xs leading-relaxed">
        {sublabel}
